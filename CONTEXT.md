...
2. **Lights** — Shows all 14 Jandy WaterColors effects as always-visible grid. Tap to activate. **Turn Off** button (top-right of "Pool Light") is now **always visible** — disabled/dimmed when the light is already off (was previously hidden). Active color synced to KV across devices. 3-second cooldown between color changes.

   **`turnOffLight()` (shared helper):** used by both the Lights-tab Off button and the Status-tab Quick Off button. Reads the freshest light device from `state.devices` at click time (not a stale `state.selectedLight` reference), no-ops with a toast if the light is already off (these buttons are off-only — they never toggle the light on; turn-on is via tapping a color), then toggles `set_aux_N`, clears the active light effect in KV, and de-highlights the color grid.
