...
| `pool_light_off` | Read fresh `get_devices` → find the color light (type 1/2) → only toggle `set_aux_N` if it's on (off-only; never turns the light on) |
...
1. **Status** — Pool temp, air temp (with animated ring), equipment row:
   ...
   - **Quick Off** row (below Setpoints) — **Pump Off** (state-aware `pool_pump_off` + starts the 30s pump cooldown; respects an active cooldown) and **Lights Off** (calls the shared `turnOffLight()`). Both are red `.btn-off` buttons in a 2-column grid.
2. **Lights** — Shows all 14 Jandy WaterColors effects as always-visible grid. Tap to activate. **Turn Off** button (top-right of "Pool Light") is **always visible and enabled** (off-only safety is enforced server-side by `pool_light_off`). Active color synced to KV across devices. 3-second cooldown between color changes.

   **`turnOffLight()` (shared helper):** used by both the Lights-tab Off button and the Status-tab Quick Off button. Calls the server-side **`pool_light_off`** smart command, which reads a *fresh* `get_devices` and only toggles the aux if the light is on — so a stale local cache can never block the command (this was the v25 bug: the off decision was made from `state.devices`, which only refreshes every 30s, so the button silently bailed with "already off"). After the call it clears the active light effect in KV and de-highlights the color grid, and toasts "Light turned off" or "Light already off" based on the `toggled` flag. The Off buttons are always visible and **always enabled** — off-only safety lives in the worker.
